...
/*   Updated: 2023/11/30 22:47:28 by mawad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*buffer_read(int fd, char *store)
{
	...
	{
		buffer = (char *)malloc(sizeof(*buffer) * ((size_t)BUFFER_SIZE + 1));
		if (!buffer)
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= 10240 || BUFFER_SIZE > 2147483647 || BUFFER_SIZE <= 0)
		return (NULL);
	store[fd] = buffer_read(fd, store[fd]);
	if (!store[fd])
		return (NULL);
	extract = extraction(store[fd]);
	if (!free_bird(extract, &(store[fd])))
		return (NULL);
	store[fd] = remaining(store[fd]);
	return (extract);
}
